...
@section('content')
    <section class="py-16 bg-white">
        <div class="container mx-auto px-6 md:px-8">
            <h1 class="text-5xl font-extrabold text-dark-gray text-center mb-12 font-heading leading-tight">Jelajahi Resep Lezat</h1>

            {{-- PHP Block for Data and Logic --}}
            @php
                // Get current search query and filter type from URL parameters
                $searchQuery = request('search', '');
                $filterType = request('filter', 'all');
                $currentPage = request('page', 1); // Get current page for manual pagination

                $allRecipes = [
                    // PASTIKAN DATA DUMMY INI SAMA PERSIS DENGAN YANG ADA DI RecipeController@show
                    (object)[
                        'title' => 'Salad Ayam Alpukat Sehat',
                        'slug' => 'salad-ayam-alpukat-sehat',
                        'image_url' => 'https://tse3.mm.bing.net/th/id/OIP.xFD7yfbqieBIkM-1qRndyAHaEy?w=237&h=180&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3', // URL placeholder baru
                    ...
                    (object)[
                        ...
                        'image_url' => 'https://tse1.mm.bing.net/th/id/OIP.H9I9EGALDCJI15ynbYM78AHaEc?w=267&h=180&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3', // URL placeholder baru
                    ...
                    (object)[
                        ...
                        'image_url' => 'https://tse3.mm.bing.net/th/id/OIP._tZfyaEbK7gRb08evSWaRgHaE8?w=260&h=180&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3', // URL placeholder baru
                    ...
                    (object)[
                        ...
                        'image_url' => 'https://tse2.mm.bing.net/th/id/OIP.vQwW7e5GC8j7x-VN5abd8gAAAA?w=224&h=180&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3', // URL placeholder baru
                    ...
                    (object)[
                        ...
                        'image_url' => 'https://tse3.mm.bing.net/th/id/OIP.ZVP-lDldzhSYpbIobV10UAHaE8?w=265&h=180&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3', // URL placeholder baru
                    ...
                    (object)[
                        ...
                        'image_url' => 'https://tse3.mm.bing.net/th/id/OIP.QMkyIhLZIk1YYSGNG9VFbwHaE7?w=291&h=193&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3', // URL placeholder baru
                    ...
                    (object)[
                        ...
                        'image_url' => 'https://tse1.mm.bing.net/th/id/OIP.88JUFluGrNjBfxnJQihEZwHaEK?w=315&h=180&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3', // URL placeholder baru
                    ...
                    (object)[
                        ...
                        'image_url' => 'https://tse4.mm.bing.net/th/id/OIP.rzBYa6xr14iuZD4ViQIoVgHaEK?w=331&h=186&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3', // URL placeholder baru
                    ...
                    (object)[
                        ...
                        'image_url' => 'https://tse1.mm.bing.net/th/id/OIP.lPNPR5Gio70zmyhFKc2IzQHaFP?w=264&h=187&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3', // URL placeholder baru
                ...
                $filteredRecipes = collect($allRecipes)->filter(function ($recipe) use ($searchQuery, $filterType) {
                    $matchesSearch = true;
                    if ($searchQuery) {
                        $matchesSearch = Str::contains(strtolower($recipe->title), strtolower($searchQuery)) ||
                                         Str::contains(strtolower($recipe->description), strtolower($searchQuery));
                    }

                    $matchesFilter = true;
                    if ($filterType === 'healthy') {
                        $matchesFilter = $recipe->is_healthy === true;
                    } elseif ($filterType === 'unhealthy') {
                        $matchesFilter = $recipe->is_healthy === false;
                    }

                    return $matchesSearch && $matchesFilter;
                })->values(); // Use values() to re-index the array after filtering

                // Manual Pagination Configuration
                $perPage = 8; // Number of recipes per page
                $totalRecipes = $filteredRecipes->count();
                $offset = ($currentPage * $perPage) - $perPage;

                // Slice the collection for the current page
                $recipesOnCurrentPage = $filteredRecipes->slice($offset, $perPage);
                ...
                $recipes = new \Illuminate\Pagination\LengthAwarePaginator(
                    ...
                    $perPage,
                    $currentPage,
                    ['path' => url()->current(), 'query' => request()->except('page')] // Ensure query parameters are kept
                );
            @endphp

            {{-- Filter & Search Form --}}
            <form action="{{ url()->current() }}" method="GET" class="flex flex-col md:flex-row justify-between items-center mb-10 space-y-4 md:space-y-0 md:space-x-4">
                <div class="w-full md:w-1/3 relative">
                    <input type="text" name="search" placeholder="Cari resep..." class="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-primary text-lg" value="{{ $searchQuery }}">
                ...
                <div class="flex space-x-4">
                    <button type="submit" name="filter" value="all" class="px-6 py-3 rounded-full text-lg font-semibold {{ $filterType === 'all' ? 'bg-primary text-white' : 'border border-primary text-primary hover:bg-primary hover:text-white' }} transition duration-300">Semua</button>
                    <button type="submit" name="filter" value="healthy" class="px-6 py-3 rounded-full text-lg font-semibold {{ $filterType === 'healthy' ? 'bg-primary text-white' : 'border border-primary text-primary hover:bg-primary hover:text-white' }} transition duration-300">Bergizi</button>
                    <button type="submit" name="filter" value="unhealthy" class="px-6 py-3 rounded-full text-lg font-semibold {{ $filterType === 'unhealthy' ? 'bg-accent text-white' : 'border border-accent text-accent hover:bg-accent hover:text-white' }} transition duration-300">Tidak Bergizi</button>
                </div>
            </form>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                @forelse ($recipes as $recipe)
                    ...
                    <a href="{{ route('resep.show', $recipe->slug) }}" class="block bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transform hover:-translate-y-2 transition duration-300 ease-in-out border border-gray-100 relative group">
                        @if($recipe->is_healthy)
                            <span class="absolute top-4 left-4 bg-secondary text-white text-sm font-bold px-3 py-1 rounded-full z-10 shadow-md">Bergizi</span>
                        @else
                            <span class="absolute top-4 left-4 bg-red-500 text-white text-sm font-bold px-3 py-1 rounded-full z-10 shadow-md">Tidak Bergizi</span>
                        @endif
                        <img src="{{ $recipe->image_url }}" alt="{{ $recipe->title }}" class="w-full h-60 object-cover object-center group-hover:scale-105 transition-transform duration-300"
                             onerror="this.onerror=null;this.src='https://placehold.co/800x600/cccccc/333333?text=Gambar+Tidak+Tersedia';">
                        <div class="p-6">
                            <h3 class="text-2xl font-semibold text-dark-gray mb-2 font-heading leading-tight">{{ $recipe->title }}</h3>
                            <p class="text-gray-700 text-base font-sans">{{ Str::limit($recipe->description, 100) }}</p>
                            <div class="flex items-center mt-3 text-sm text-gray-500">
                                <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M10 12.586l4.243 4.243A1 1 0 0015.657 16L12 12.343l4.243-4.243A1 1 0 0016 7.343L12.343 11l4.243-4.243a1 1 0 00-1.414-1.414L11 9.586l-4.243-4.243A1 1 0 004.929 5.071L9.586 10l-4.243 4.243A1 1 0 005.071 15.657L10 11.071z"></path></svg>
                                <span>{{ $recipe->prep_time }}</span>
                            </div>
                            <div class="flex items-center text-yellow-500 mt-2">
                                @for ($i = 0; $i < round($recipe->average_rating); $i++)
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.538 1.118l-2.8-2.034a1 1 0 00-1.176 0l-2.8 2.034c-.783.57-1.838-.197-1.538-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.929 8.72c-.783-.57-.381-1.81.588-1.81h3.462a1 1 0 00.95-.69l1.07-3.292z"></path></svg>
                                @endfor
                                <span class="ml-1 text-gray-600 text-sm">({{ $recipe->ratings_count }} ulasan)</span>
                            </div>
                        </div>
                    </a>
                @empty
